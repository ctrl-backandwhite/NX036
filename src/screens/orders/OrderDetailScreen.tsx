      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <View className="mx-5 mt-4 bg-white rounded-2xl p-4 border border-slate-100">
          ...
          {order.estimatedDelivery && order.status !== 'delivered' && order.status !== 'cancelled' && (
            <Text className="text-sm text-slate-600 mt-1">
              📅 Entrega estimada: <Text className="font-semibold">{order.estimatedDelivery}</Text>
            </Text>
          )}
        </View>
        ...
        {order.trackingNumber && !['delivered', 'cancelled', 'refunded'].includes(order.status) && (
          <TouchableOpacity
            ...
            className="mx-5 mt-3 bg-indigo-600 rounded-2xl p-4 flex-row items-center justify-between">
        ...
        <View className="mx-5 mt-3 bg-white rounded-2xl p-4 border border-slate-100">
          <Text className="text-base font-semibold text-slate-800 mb-2">📍 Dirección de envío</Text>
        ...
        {/* Payment */}
        <View className="mx-5 mt-3 bg-white rounded-2xl p-4 border border-slate-100">
          <Text className="text-base font-semibold text-slate-800 mb-2">💳 Método de pago</Text>
        ...
        <View className="mx-5 mt-3 mb-6 bg-white rounded-2xl p-4 border border-slate-100">
          <Text className="text-base font-semibold text-slate-800 mb-3">💰 Resumen</Text>
          ...
          {order.loyaltyPointsEarned > 0 && (
            <View className="flex-row items-center mt-3 bg-amber-50 px-3 py-2 rounded-lg">
              <Text className="text-xs text-amber-800">🏆 +{order.loyaltyPointsEarned} puntos ganados</Text>
            </View>
          )}
        </View>
      </ScrollView>
